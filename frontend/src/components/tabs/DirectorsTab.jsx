...
export default function DirectorsTab({ companyId, client }) {
  ...
  useEffect(() => { fetchRecords(); fetchIndivs() }, [companyId])
  ...
  const del = async rec => {
    ...
    await directorsApi.delete(rec.company_client_id, rec.individual_client_id); fetchRecords()
  }

  const exportHead = ['Name', 'DIN', 'Designation', 'Date of Appointment', 'Date of Cessation', 'Status', 'KMP', 'Notes']
  const exportRows = records.map(r => [r.individual_name, r.din, r.designation, r.date_of_appointment, r.date_of_cessation, r.is_active ? 'Active' : 'Inactive', r.is_kmp ? 'Yes' : 'No', r.notes])
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {records.length > 0 && client && (
            <ExportMenu
              small label="Export"
              onExportPDF={() => exportSectionPDF({ client, title: 'Directors', head: exportHead, rows: exportRows })}
              onExportExcel={() => exportSectionExcel({ client, title: 'Directors', head: exportHead, rows: exportRows })}
            />
          )}
